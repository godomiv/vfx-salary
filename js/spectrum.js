...
// Безопасное экранирование для HTML-контекста
function escSpec(s) {
  if (!s) return '';
  return String(s)
    .replace(/&/g, '&amp;')
...
function drawSpectrum(){
  ...
  filtered.forEach(d=>{
    ...
    // Детерминированный шум по id — точки не прыгают при перерисовке
    const jitter = ((d.id * 2654435761) % 1000) / 1000 - 0.5;
    const y=24+best*rowH+rowH/2+jitter*3;
  ...
  // Легенда — escSpec защищает label от XSS
  ...
  items.forEach(i=>legEl.innerHTML+=`<div class="sl-item"><div class="sl-dot" style="background:${escSpec(i.color)}"></div>${escSpec(i.label)}</div>`);
  // Tooltip — все поля экранированы через escSpec
  ...
  canvas.onmousemove=function(e){
    ...
    if(hit){
      const d=hit.d;
      const sw=d.software?d.software.split(/[,/]/).map(s=>s.trim()).filter(Boolean).slice(0,4).map(escSpec).join(', '):'';
      const proj=d.projects?d.projects.split(/[,/]/).map(s=>s.trim()).filter(Boolean).slice(0,3).map(escSpec).join(', '):'';
      specTip.innerHTML=`<b>${escSpec(d.city)}</b> · <span style="color:${LEVEL_COLORS[d.level]}">${escSpec(d.level)}</span><br>${escSpec(d.dept)}<br><b style="color:var(--accent)">${fmtSalary(d.salary)}</b> · ${expLabel(d.exp)}<br>${EMP_LABELS[d.emp]||'—'} · ${FMT_LABELS[d.fmt]||'—'}${proj?`<br><span style="color:#6b8aff;font-size:.7rem">${proj}</span>`:''}${sw?`<br><span style="color:#7878a0;font-size:.7rem">${sw}</span>`:''}`;
